...
- [x] 第 0 階段「先別讓人失望」：真正的 CSV 匯入、移除假的分享連結、繁中／英／日介面、機場中日文搜尋別名（PR #98）
- [x] 第 1 階段「輸入簡單到不用想」：
  - 貼上訂位確認信／行程表自動找出航班（`app/text-import.js`，支援 `TPE → NRT`、`台北 (TPE) … 東京 (NRT)`、中英日期格式、無年份日期、轉機串接）
  - 掃描登機證截圖（瀏覽器 BarcodeDetector 讀 PDF417／Aztec／QR，解析 IATA BCBP；不支援的瀏覽器會提示改用貼上）
  - 所有匯入來源共用同一個驗證與預覽（`analyzeDrafts`），確認前不寫入
  - 新增表單只剩出發、抵達、日期必填；航空公司與航班編號選填，其餘收進「更多資訊」
  - 出發地預設為最常出發的機場，不再預填 SFO → JFK
  - 空白地圖顯示新手引導卡：手動新增／貼上訂位信／匯入試算表；第一趟航班加入時有慶祝提示
...
第 1 階段後 Meridiel 初始 JS 為 293 KB／300 KB。第 2 階段（分享圖卡版型、動態影片）前必須先把匯入流程（CSV、貼上、掃描）與分享模組改為按需載入。

- [ ] 匯入面板 code-split，開啟「新增航班」時才載入
- [ ] 預算測試保留至少 30 KB 餘裕
