...
describe('EkiSpell deployment release', () => {
  ...
  it('ships every browser module and resolves all local imports', () => {
    ...
    const files = ['app.js', 'real-data.js', 'map.js', 'routes.js', 'route-panel.js', 'route-options.js', 'metro-journey.js', 'journey-panel.js', 'journey-review.js','receipt-check.js','receipt-panel.js', ...readdirSync(resolve(root, 'dist')).filter(f => f.endsWith('.js')).map(f => `dist/${f}`)];
