...
try {
  ...
  for (const name of ['map.js','routes.js','route-panel.js','route-options.js','metro-journey.js','journey-panel.js','metro-evidence.json','metro-topology.json','coordinates.json','vendor']) await cp(join(root, 'integrations/ekispell', name), join(temporary, 'site', name), {recursive:true});
