/* 由 scripts/build-meridiel.mjs 從 globe.jsx 編譯產生，請勿手動編輯 */
(function () {
/* ============================================================
...
function GlobeView({
  ...
  useEffect(() => {
    ...
    })(elRef.current).backgroundColor("rgba(0,0,0,0)").showAtmosphere(true).atmosphereColor(P.atmosphere).atmosphereAltitude(0.17).showGraticules(true);
    ...
    world.arcStartLat(d => d.from.lat).arcStartLng(d => d.from.lng).arcEndLat(d => d.to.lat).arcEndLng(d => d.to.lng).arcColor(d => isHot(d) ? [palRef.current.arcSelA, palRef.current.arcSelB] : [palRef.current.arcA, palRef.current.arcB]).arcStroke(d => isHot(d) ? 2.2 : 0.55)
    ...
    .arcDashLength(d => isHot(d) ? 0.3 : 1).arcDashGap(d => isHot(d) ? 1.2 : 0).arcDashInitialGap(0).arcDashAnimateTime(d => isHot(d) ? 1600 : 0).arcsTransitionDuration(700).arcLabel(d => `<div class="globe-tip"><b>${d.from.code} → ${d.to.code}</b><span>${d.from.city} – ${d.to.city}</span><span>${d.miles.toLocaleString()} mi · ${d.year}</span></div>`).onArcClick(d => onSelect && onSelect(d.id));

    // airport points
    world.pointLat(d => d.lat).pointLng(d => d.lng).pointColor(() => palRef.current.point).pointAltitude(0.012).pointRadius(d => focusRef.current && (d.code === focusEnd(0) || d.code === focusEnd(1)) ? 0.4 : 0.22).pointsMerge(false).pointsTransitionDuration(700).pointLabel(d => `<div class="globe-tip"><b>${d.code}</b><span>${d.city}, ${d.country}</span></div>`).onPointClick(d => {
      // clicking an airport highlights the first connected flight
      const match = flightsRef.current.find(f => (f.from && f.from.code) === d.code || (f.to && f.to.code) === d.code);
    ...
    world.ringColor(() => t => `rgba(210,154,60,${Math.sqrt(1 - t)})`).ringMaxRadius(4).ringPropagationSpeed(2.2).ringRepeatPeriod(900);
    ...
    const drawLand = features => {
      world.polygonsData(features.filter(f => f.properties && f.properties.ISO_A2 !== "AQ")).polygonCapColor(() => palRef.current.land).polygonSideColor(() => "rgba(120,95,55,0.25)").polygonStrokeColor(() => palRef.current.landEdge).polygonAltitude(0.008);
    };
    const fetchLand = attempt => {
      fetch(COUNTRIES_URL).then(r => r.ok ? r.json() : Promise.reject(new Error("HTTP " + r.status))).then(geo => {
        drawLand(geo.features);
        try {
          localStorage.setItem(LAND_CACHE_KEY, JSON.stringify(geo.features));
        } catch (e) {/* storage full — fine */}
      }).catch(() => {
        if (attempt < 2) setTimeout(() => fetchLand(attempt + 1), 1400);
      });
    };
    ...
    try {
      ...
      if (cachedLand) {
        drawLand(JSON.parse(cachedLand));
        drewLandFromCache = true;
      }
    ...
    const watchdog = setInterval(() => {
      ...
      if (f === lastFrame) {
        try {
          world._animationCycle && world._animationCycle();
        } catch (e) {}
      }
    ...
    const onVis = () => {
      if (!document.hidden) {
        try {
          world.resumeAnimation && world.resumeAnimation();
        } catch (e) {}
      }
    };
  ...
  useEffect(() => {
    ...
    world.arcColor(world.arcColor()).pointColor(() => P.point).polygonCapColor(() => P.land).polygonStrokeColor(() => P.landEdge);
  }, [theme]);
  ...
  useEffect(() => {
    ...
    if (!world) {
      ...
      console.warn("Meridiel: flights changed before the globe was ready — will draw on init", {
  ...
  useEffect(() => {
    ...
    const world = globeRef.current;
    if (!world) return;
    world.arcColor(world.arcColor()).arcStroke(world.arcStroke()).arcDashLength(world.arcDashLength()).arcDashGap(world.arcDashGap()).arcDashAnimateTime(world.arcDashAnimateTime());
  }, [selectedId]);
  ...
  useEffect(() => {
    ...
    world.arcColor(world.arcColor()).arcStroke(world.arcStroke()).arcDashLength(world.arcDashLength()).arcDashGap(world.arcDashGap()).arcDashAnimateTime(world.arcDashAnimateTime()).pointRadius(world.pointRadius());
